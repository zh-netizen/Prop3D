...
#使用RMSE作为指标 注重均值
# def train_loop(model, loader, optimizer, device):#训练模型，均方误差作为判断标准
#     model.train()
#
#     losses = []
#     epoch_loss = 0
...
def train_loop(model, loader, optimizer, device):
    ...
    with tqdm.tqdm(total=len(loader), desc=progress_format.format(0)) as t:
        for i, data in enumerate(loader):
            ...
            output = model(feature)
            # Use L1 loss for MAE
            batch_losses = F.l1_loss(output, label, reduction='none')
            batch_losses_mean = batch_losses.mean()
            batch_losses_mean.backward()
            optimizer.step()
            # stats
            # Update epoch_loss with a running average
            epoch_loss += (batch_losses_mean.item() - epoch_loss) / float(i + 1)
            # Collect individual batch losses for final average calculation
            losses.extend(batch_losses.detach().cpu().numpy())  # Detach and move to CPU for memory efficiency
            t.set_description(progress_format.format(epoch_loss))  # Updated to show current running average loss
            t.update(1)
...
def Test(model, loader, device):
    ...
    for data in loader:
        ...
        label = data['label'].to(device).to(torch.float32)
        output = model(feature)
        batch_losses = F.l1_loss(output, label, reduction='none')#使用l1loss，也就是MAE
        losses.extend(batch_losses.tolist())
        ids.extend(data['id'])
        y_true.extend(label.tolist())
        y_pred.extend(output.tolist())
        ...
        # MAE的最终值应该是所有损失的平均值
    final_mae = np.mean(losses)

    results_df = pd.DataFrame(
        np.array([ids, y_true, y_pred]).T,
        columns=['structure', 'true', 'pred'],
    )
    return final_mae, results_df
...
def train(args, device, test_mode=False):
    ...
    # Save config
    with open(os.path.join(args.output_dir, 'config.json'), 'w') as f:#将参数写道config文件中，方便保存
        json.dump(args.__dict__, f, indent=4)

    np.random.seed(args.random_seed)#随机种子，确保实验的可重复性
    torch.manual_seed(args.random_seed)

    train_dataset = LMDBDataset(
        os.path.join(args.data_dir, 'train'),
        transform=CNN3D_TransformSMP(args.label_name, random_seed=args.random_seed))#将mdb数据库中的数据取出来，进行序列化
    ...
    for data in train_loader:#处理数据的格式
...
if __name__=="__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='/home/zhanghuan/atom3d/examples/smp/data/qm9/data')
    parser.add_argument('--mode', type=str, default='test',
                        choices=['train', 'test'])
    # parser.add_argument('--output_dir', type=str, default=os.environ['LOG_DIR'])
    parser.add_argument('--output_dir', type=str, default='/home/zhanghuan/atom3d/examples/smp/output_log')
    parser.add_argument('--unobserved', action='store_true', default=False)

    parser.add_argument('--label_name', type=str, default='u298_atom',
    ...
    parser.add_argument('--random_seed', type=int, default=int(92467))

...
grid_x, grid_y, grid_smiles, sample_shape = read_griddata('/home/zhanghuan/atom3d/examples/esol/grid3Dmols_delaney')
print(len(grid_x))
...
grid=create_grid_from_coordinates(grid_x[0],sample_shape)
print(grid[19,26,22,1],grid[18,25,24,1],grid[11,22,22,3])
'''
你可以通过字典的键来访问数据，例如：
...
#         running_mae += torch.abs(outputs - labels).mean().item()  # 计算当前批次的 MAE
#
#     # 计算并返回平均损失和 MAE
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for data in tqdm(train_loader, desc="Training"):
        ...
        # 累积损失和 MSE
        running_loss += loss.item()
        mse = torch.nn.functional.mse_loss(outputs, labels).item()  # 计算当前批次的 MSE
        running_mse += mse

    # 计算平均损失和 RMSE
    avg_loss = running_loss / len(train_loader)
    rmse = (running_mse / len(train_loader)) ** 0.5  # RMSE 是 MSE 的平方根
...
def validate(model, val_loader, criterion, device):
    ...
    # 计算并返回平均损失、MAE 和 RMSE
    avg_loss = running_loss / len(val_loader)
    avg_mae = running_mae / len(val_loader)
    avg_rmse = running_rmse / len(val_loader)
...
# 定义损失函数（使用 MAE）和优化器（Adam）
criterion = torch.nn.L1Loss()  # 使用 MAE 作为损失函数
...
for epoch in range(num_epochs):
    ...
    print(f"Train Loss: {train_loss:.4f}, Train RMSE: {train_mae:.4f}")

    # 验证模型
    val_loss, val_mae,val_rmse = validate(model, val_loader, criterion, device)
    print(f"Validation Loss: {val_loss:.4f}, Validation MAE: {val_mae:.4f},Validation RMSE: {val_rmse:.4f}")

    test_loss, test_mae,test_rmse = mytest(model, test_loader, criterion, device)
